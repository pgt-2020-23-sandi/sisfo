	...
	
{
	...
	public function student_login_check($nim,$password)
	{ 
		$st_login_check = "SELECT  * FROM mahasiswa WHERE nim = '$nim' and password='$password'";
		$st_login_run = $this->connectdb->query($st_login_check);
		$st_login_num = $st_login_run->num_rows;
		return $st_login_num;
	}

	public function student_info_select($nim)
	{
		$student_info_sel = "SELECT * from mahasiswa where nim='$nim'";
	...
	public function teacher_info_instudent($nim)
	{
		$teacher_info_instudent_select = "SELECT * FROM nilaiakademik WHERE nim='$nim'";
	...
	public function student_password_change($st_password_update,$nim)
	{
		$student_password_update = "update mahasiswa set password='$st_password_update' where nim='$nim'";
